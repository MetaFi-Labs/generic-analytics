...
interface VaultItemProps {
  mandatory: VaultItemMandatoryProps;
  internal?: VaultItemInternalProps;
}

export interface VaultItemMandatoryProps {
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ backgroundColor: props.mandatory.assetMetadata.color + '20' }}>
          <img src={props.mandatory.assetMetadata.iconSrc} />
        </div>
        <div>
          <h3 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">{props.mandatory.assetMetadata.symbol}</h3>
          <p className="text-xs text-zinc-500 dark:text-zinc-400">{props.mandatory.assetMetadata.name}</p>
        </div>
      </div>
      <div className="space-y-1">
        ...
        <div className="flex justify-between items-center">
          ...
          <span className="text-sm font-medium text-zinc-700 dark:text-zinc-300">
            ${props.mandatory.price.toFixed(4)}
          </span>
        </div>
        ...
        {props.internal && (
          <div className="pt-2 border-zinc-200 dark:border-zinc-800 space-y-3">
